...
export default function TelemetryDemo() {
  ...
  return (
    <section id="demo" className="py-24 relative bg-slate-50/50 border-y border-slate-200">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex p-1.5 rounded-2xl bg-slate-100 border border-slate-200 overflow-x-auto max-w-full">
            <button
        ...
        {/* Tab Showcase Card */}
        <div className="max-w-4xl mx-auto rounded-2xl border border-slate-200 p-6 sm:p-8 bg-white shadow-xl">
          ...
          {activeTab === "perms" && (
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100 mb-6">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded bg-violet-50 text-violet-700 border border-violet-200 font-semibold">
                      Pro Feature
                    </span>
                  </h3>
              ...
              <div className="space-y-3 font-mono text-xs">
                <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-slate-900 font-semibold">wp-config.php</span>
                      <span className="block text-[11px] text-slate-500 font-sans">Database credentials, salts & security keys</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`px-2.5 py-1 rounded font-bold ${
                      permsFixed ? "bg-emerald-100 text-emerald-800 border border-emerald-300" : "bg-rose-100 text-rose-800 border border-rose-300"
                    }`}>
                ...
                <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <FileCode className="h-4 w-4 text-slate-500" />
                    <div>
                      <span className="text-slate-900 font-semibold">wp-content/uploads</span>
                ...
                <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <FileCode className="h-4 w-4 text-slate-500" />
                    <div>
                      <span className="text-slate-900 font-semibold">.htaccess / .user.ini</span>
                      <span className="block text-[11px] text-slate-500 font-sans">Server routing and PHP runtime overrides</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-1 rounded font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                      0644 (Standard)
                    </span>
                  </div>
                </div>
              ...
              {permsFixed && (
                <div className="mt-4 p-3 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center gap-2 text-xs text-emerald-800 font-medium">
            ...
            <div>
              <div className="pb-6 border-b border-slate-100 mb-6">
                <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200 font-semibold">
                    Pro Active
                  </span>
                </h3>
                <p className="text-xs text-slate-500 mt-1">
              ...
              <div className="space-y-3 text-xs">
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <span className="self-start sm:self-auto px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-1">
                    <CheckCircle2 className="h-3.5 w-3.5" />
                    Safe to Update
                  </span>
                </div>

                <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 font-bold text-slate-900 text-sm">
                      <span>Custom Legacy PDF Invoicer</span>
                      <span className="text-xs text-slate-500 font-normal">v2.1 &rarr; v2.2</span>
                    </div>
                    <span className="text-amber-800 text-xs mt-0.5 block font-mono">
                      Deprecated call: utf8_encode() at class-pdf.php:124
                    </span>
                  </div>
                  <span className="self-start sm:self-auto px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-300 flex items-center gap-1">
            ...
            <div>
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80 space-y-3">
                <div className="flex items-center justify-between text-xs pb-3 border-b border-slate-200/60">
                  <span className="text-slate-500 font-mono">Your Admin View:</span>
                  <span className={`font-bold ${safeModeActive ? "text-amber-700" : "text-slate-700"}`}>
                    {safeModeActive ? "Plugins Isolated (Clean Sandbox)" : "Normal Admin View"}
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs pb-3 border-b border-slate-200/60">
                  <span className="text-slate-500 font-mono">Live Visitors (Checkout/Traffic):</span>
                  <span className="font-bold text-emerald-700">100% Unaffected (Normal Traffic)</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-mono">Database active_plugins table:</span>
                  <span className="font-bold text-indigo-700">Never Modified (Zero Risk of Lockout)</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === "ide" && (
            <div>
              ...
              <div className="p-4 rounded-xl bg-slate-900 font-mono text-xs text-slate-200 border border-slate-800 shadow-inner">
                <div className="text-slate-400 mb-2"># phpinfo() WP Automated Health Check Guard</div>
                ...
                <div className="text-slate-400 mt-3"># If an invalid directive triggers HTTP 500:</div>
                <div className="text-violet-300 font-semibold">✓ Auto-Rollback kicks in instantly (Restores last known safe config)</div>
              </div>
            </div>
          )}
